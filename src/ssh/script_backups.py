...
def parse_args():
    p = argparse.ArgumentParser(description="Backup running configuration from devices")
...
def save_results(outdir: pathlib.Path, host_name: str, results: dict, save_raw: bool):
    if "__error__" in results:
        # si erreur -> fichier .ERROR.txt
        (outdir / f"{safe_name(host_name)}__ERROR.txt").write_text(
            results["__error__"], encoding="utf-8"
        )
        return
    ...
    for cmd, out in results.items():
        if cmd.startswith("__"):
            continue
        # si une seule commande -> host.cfg
        if len([k for k in results if not k.startswith("__")]) == 1:
            (outdir / f"{safe_name(host_name)}.cfg").write_text(out, encoding="utf-8")
        else:
            (outdir / f"{safe_name(host_name)}__{safe_name(cmd)}.txt").write_text(
                out, encoding="utf-8"
            )
...
def main():
    args = parse_args()
    # dossier final = racine/outdir + YYYYMMDD
    batch_dir = pathlib.Path(args.outdir) / today()
    batch_dir.mkdir(parents=True, exist_ok=True)
    # (on remplira ici)
    print(f"Create folder for the backups : {batch_dir}")
    hosts = load_inventory(args.inventory)
    print("Inventaire:", [h["name"] for h in hosts])
    for h in hosts:
        print(f"===> {h['name']} ({h['host']})")
        results = run_commands_on_host(h, ["show running-config"])
        save_results(batch_dir, h["name"], results, args.save_raw)

    print(f"✔ Done. Result exported in {batch_dir}")
